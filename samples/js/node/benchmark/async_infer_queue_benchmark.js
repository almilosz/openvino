...
async function benchmarkPromiseAll(model, datasetSize) {
  ...
  let inputLayer = model.inputs[0];
  const shape = model.inputs[0].getShape();
  const dataset = Array.from(
    { length: datasetSize + 10 }, () => generateImage(shape),
  );

  const inferQueue = new ov.AsyncInferQueue(model, 4);
  inferQueue.setCallback((ireq,i,err) => {});

  // Warm up
  for (let i = 0; i < 10; i++) {
    await inferQueue.startAsync({ [inputLayer.getAnyName()]: dataset[i] }, i);
  }

  const hrs = process.hrtime();
  const promises = [];
  for (let i = 1; i < datasetSize; i++) {
    promises.push(inferQueue.startAsync({ [inputLayer.getAnyName()]: dataset[i] }, i));
  }
...
async function benchmark(model, datasetSize) {
  ...
  const outputPath = path.join(__dirname, 'inferqueue_times.json');
...
async function main() {
  ...
  const compiledModel = await core.compileModelSync(args[2], deviceName, latency);
